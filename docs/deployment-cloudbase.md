...
状态：CloudBase 服务已从 Git 构建上线；启动时从私有 COS 下载并校验两份固定快照，七维诊断可用。2026-09-26 公网验收 `py scripts/smoke_public.py` 为 21/21。产品数据截至 2026-08-31，固定快照不自动刷新；早期 cosfs 挂载失败记录保留在文末历史小节。见[决策 0020](decisions/0020-snapshot-download-fallback.md)。
...
2. 只把逐字段核准、许可允许保存的**发布快照**上传到 CloudBase 云存储。当前私有对象是 `snapshots/profit_cash_002466.json` 与 `snapshots/product_snapshot_002466.json`；记录文件摘要、下载时点、报告期和对象名。原始扶摇/iFinD 批量响应不作为产品快照上传。
3. 云托管服务启动时用只读子账号凭据从私有桶下载两份快照到容器本地，校验摘要、版本、标的和截止日后供只读诊断使用。环境变量名称见下节及[`.env.example`](../.env.example)。更新桶内对象后须重启实例以重新下载，并复核快照 ID 与公网主链路；不使用 CloudBase 存储挂载。
...
- 构建入口为仓库根目录 `Dockerfile`：Gunicorn 运行 `webapp:app`，监听 `PORT`（默认 8080），超时 60 秒；健康检查 `GET /healthz`；诊断接口 `GET /api/bootstrap`、`POST /api/chat`（决策 0015、0016、0022）。服务端配置包括 `DEEPSEEK_API_KEY`、`DEEPSEEK_BASE_URL`、`DEEPSEEK_MODEL`、`DIAGNOSIS_COS_BUCKET`、`DIAGNOSIS_COS_REGION`、`DIAGNOSIS_COS_PREFIX`、`DIAGNOSIS_COS_SECRET_ID`、`DIAGNOSIS_COS_SECRET_KEY`、`DIAGNOSIS_SNAPSHOT_DIR`；本地文件路径可用 `DIAGNOSIS_SNAPSHOT_PATH`、`DIAGNOSIS_PRODUCT_SNAPSHOT_PATH` 指定。
- DeepSeek 与 COS 只读凭据只放服务端环境变量；提问路径不需要扶摇/iFinD 凭据，前端不读取私有对象。
- 启动时验证下载结果和快照内容摘要、标的、版本与截止日。异常时服务报告“快照不可用”；不得把缺失当零，也不得退回构造样本。
- 公开 URL 验收：页面可打开；七维与四类窄问题可操作；结论及计算输入可钻取；显示报告期和快照时间；缺失/冲突/不适用状态正确；直接访问云存储快照被拒绝。新版本部署后复核两份快照 ID 与七维状态。
- 记录真实环境 ID、服务名、部署版本和 URL 时，应写入 README 与测试记录；不要在这些记录中写访问密钥。公开展示许可已由用户确认，且不设接口限流（决策 0017）。

Git 构建只能读取已推送的提交；每次发布前检查代码、文档、测试和敏感内容。当前公网地址为 <https://lithium-diagnosis-319862-8-1496111595.sh.run.tcloudbase.com>，首次浏览器访问测试域名可能需要确认风险提醒。最近一次公网验收结果见[测试说明](test-plan.md)。

## 历史部署排障记录（2026-09-26 13:40 北京时间起）

以下“当前”“未完成”“待办”均为当时的交接状态，已由上文的启动下载方案及后续公网验收取代；保留记录用于追溯失败原因，不作为现行操作步骤。
