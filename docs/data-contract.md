...
- **估值口径门禁**：iFinD 估值 TTM/MRQ 口径已核实，见[决策 0014](decisions/0014-ifind-valuation-definitions.md)：分子为总市值；`PE(TTM)` 分母为归母净利润 TTM（滚动 4 季，基准日=报表公告日期），`PE(MRQ)` 为最新一期累计归母净利润×年化系数（半年报×2），`PB(MRQ)` 分母为最新一期归母净资产，`PS/PCF(TTM)` 分别用收入/经营现金流 TTM。分母为负时返回负 PE，产品须标 `not_applicable`。分子绝对水平仍约有 5% 未解释，历史分位只在 iFinD 自身序列内计算，不与扶摇或其他来源混算；年报期与季报期年化系数待补验。
