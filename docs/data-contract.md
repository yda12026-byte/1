...
- **估值口径门禁**：iFinD 估值 TTM/MRQ 口径已核实，见[决策 0014](decisions/0014-ifind-valuation-definitions.md)：分子为市值（基准最接近总市值）；`PE(TTM)` 分母为归母净利润 TTM（滚动 4 季，基准日=报表公告日期），`PE(MRQ)` 为最新一期累计归母净利润×年化系数（一季报 ×4、半年报 ×2、三季报 ×4/3、年报 ×1，均已验证），`PB(MRQ)` 分母为最新一期归母净资产（iFinD 直接提供，官方半年报可复核），`PS/PCF(TTM)` 分别用收入/经营现金流 TTM。分母为负时返回负 PE，产品须标 `not_applicable`。PE/PB 所用市值的绝对基准仍比独立查询的“总市值”约大 5%、未完全解释，故历史分位只在 iFinD 自身序列内计算，不与扶摇或其他来源市值/估值混算。
- **公告/新闻待定**：呈现方式未决，决定前检索结果不入正式事件事实，只作待核线索。
