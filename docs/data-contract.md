...
- **估值图**：涉及估值维度时返回 `valuation_chart`：PB 与 PE(TTM) 逐交易日序列、截止日分位（与 `*_percentile` 证据同一算法）、33.3%/66.7% 分位对应的倍数边界、半年报披露日（TTM 分母切换）；序列含非正值时不给边界。
- **锂价图**：涉及行业维度时返回 `lithium_chart`：四类锂价按各自首个交易日指数化（Decimal，两位小数）并附原值与单位，每条序列对应行业维度的 `lithium_series:{key}` 证据（记录剔除的非交易日行数）。
