...
项目骨架已建立。目标个股确定为**天齐锂业（002466.SZ）**；选择理由见[决策记录](docs/decisions/0003-target-stock.md)。DeepSeek、扶摇和 iFinD 的本地访问已通过最小调用验证。除 HTML 前端外，后续实现优先使用 Python；具体 Web 框架与部署地址尚待确定。此 README 会随实现补充启动方式、数据来源、AI 的角色和已知边界。
