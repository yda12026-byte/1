...
- 完整产品快照由 `py scripts/build_product_snapshot.py` 从一年原始文件一次性生成到 `data/cache/product_snapshot_002466.json`（不覆盖；`DIAGNOSIS_PRODUCT_SNAPSHOT_PATH` 可覆盖路径，默认与两字段快照同目录）。`src/diagnosis/dimensions.py` 实现估值、财务趋势、行情、行业四维，`dimensions_extra.py` 实现经营质量与风险（读取官方半年报摘录，决策 0018）；事件只列待核线索。结论锚点不得含数字或禁词，且必须出现在回退文案中（测试强制）。测试用合成快照 `tests/product_fixture.py`，不得读取真实 `data/cache`。见决策 0016。
