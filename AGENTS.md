...
- 已实现的证据和结论均记录公司专属四档优先级（字段 ID、档位、理由、配置版本）；计算证据记录输入优先级来源，LLM 受限翻译读取优先级。优先级只指导计算编排与解读详略，不改变数值公式或证据质量状态；七维已按优先级编排已核准指标，未核准项保持缺口。见 `docs/decisions/0011-evidence-priority-propagation.md`。
...
- 两字段快照的一次性下载入口是 `py scripts/download_data.py`，发布到 Git 忽略的 `data/cache/`；文件已存在时不再下载或覆盖。`py scripts/diagnose_profit_cash.py` 只读该快照，可用 `DIAGNOSIS_SNAPSHOT_PATH` 指定私有文件。完整七维产品快照由 `py scripts/build_product_snapshot.py` 从已核准原始文件生成，含扶摇、iFinD、官方半年报和巨潮公告数据；快照均不自动过期，见决策 0006、0016、0022。
- 用户已确定使用 CloudBase 个人版：云托管从 Git 构建 Python Web 代码，已核准的固定快照单独存 CloudBase 云存储，运行时由服务用只读密钥从私有桶 `snapshots/` 下载到容器本地并校验摘要（`DIAGNOSIS_COS_*` 环境变量，决策 0020；存储挂载因平台 cosfs 失败已弃用）。禁止把快照打进 Git/镜像或让浏览器直连云存储。公网地址已上线并通过 21 项验收（`py scripts/smoke_public.py`）。见 `docs/decisions/0012-cloudbase-git-and-storage-deployment.md` 和 `docs/deployment-cloudbase.md`。
...
- `config/diagnosis_fields.json` 已映射当前 72 条候选字段到七维、来源、候选状态及计算依赖；可读对照见 `docs/question-routing-map.md`，用 `py scripts/export_question_routing_md.py --check` 核对。`py scripts/route_question.py "问题"` 只展示路由。`py scripts/diagnose_question.py "问题"` 可用固定两字段快照回答净利润正负、经营现金流正负、两者比值与方向关系四类窄问题；Web 可用产品快照回答七维及组合问题。未覆盖年份、两项绝对差额等问法仍为 `planned`，不可把路由候选项称作已核准证据。候选表改动后可运行 `py scripts/build_route_catalog.py` 重建并审阅差异；见决策 0008、0009。
...
- 完整产品快照由 `py scripts/build_product_snapshot.py` 从一年原始文件一次性生成到 `data/cache/product_snapshot_002466.json`（不覆盖；`DIAGNOSIS_PRODUCT_SNAPSHOT_PATH` 可覆盖路径，默认与两字段快照同目录）。`src/diagnosis/dimensions.py` 实现估值、财务趋势、行情、行业四维，`dimensions_extra.py` 实现经营质量与风险（读取官方半年报摘录，决策 0018），`dimension_events.py` 实现巨潮公告重要事件（决策 0022）；新闻只列待核线索。结论锚点不得含数字或禁词，且必须出现在回退文案中（测试强制）。测试用合成快照 `tests/product_fixture.py`，不得读取真实 `data/cache`。见决策 0016。
- 重要事件（决策 0022）：`py scripts/download_cninfo_events.py` 下载巨潮公告清单与八类重要事项 PDF，`py scripts/extract_cninfo_events.py` 程序摘录（需 PyMuPDF），`py scripts/record_event_spotcheck.py <ID>=consistent` 记录人工抽查；三者输出都在 Git 忽略的一年原始目录。分类规则在 `src/diagnosis/events.py`，维度在 `src/diagnosis/dimension_events.py`；快照的 `events` 块可选，旧快照下事件退回线索。抽查结果变化后需重建并重新上传产品快照。
...
- 一年原始数据的核准口径已由用户确定并记入[决策 0013](docs/decisions/0013-annual-data-approval-decisions.md)：扶摇按累计值标注、负基期同比用绝对基期并注明、估值/行业/价格按实际交易日过滤、EDB 锂价作行业环境证据、正式同行组为赣锋/中矿资源/永兴材料、iFinD 估值 TTM/MRQ 已核实、`f059` 显示“该请求范围无匹配事件”、`f048` 保留为历史数据缺口。公告按决策 0022 使用巨潮官方清单与原文，新闻仍为线索。原始文件本身不是产品证据；已核准部分发布为固定快照。
